...
const checkOrderStatus_1 = require("../middlewares/checkOrderStatus");
...
describe('Order Handler', () => {
    beforeAll(function () {
        spyOn(orders_1.store, 'create').and.returnValue(Promise.resolve(orderSample));
        // mock response with active order for show() in the checkOrderStatus middle ware
        spyOn(checkOrderStatus_1.orderStore, 'show').and.returnValue(Promise.resolve({
            id: 7,
            order_status: 'active',
    ...
    it('create route should return an added order record', async () => {
        const response = await request
            ...
            .send(orderSample);
        expect(orders_1.store.create).toHaveBeenCalledWith({
            id: 1,
            order_status: 'complete',
            user_id: '2',
        });
    ...
    it('showByUser route should show a list of order records according to input user id', async () => {
        const response = await request
            .get('/orders/users/2')
            .set('Authorization', `Bearer ${token}`);
        expect(orders_1.store.showByUser).toHaveBeenCalledWith('2');
        expect(response.status).toBe(200);
