...
describe('Product Handler', () => {
  ...
  it('index route should return a list of items', async () => {
    ...
    expect(response.body[0].product_name).toEqual(
      'Nudie Nothing But Aloha Blend 400ml'
    );
  });
  ...
  it('show route should return a record of the product with id 10', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.product_name).toEqual(
      'Perfection Chinese Wombok Baby whole each'
    );
  });
  ...
  it('create route should return a record of the added product', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.product_name).toEqual(
      'Belgian Chocolate Layer Cake 170g'
    );
  });
